...
Suite responsibilities follow existing package boundaries.

The CLI's suite support is execution-only through `run --suite <slug>`. Suite CRUD remains in the web app and Convex backend, while shared suite and run-selector contracts live in `@workspace/contracts`.
